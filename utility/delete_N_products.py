...
parser = argparse.ArgumentParser(description="Send N delete commands to URL/product at")
parser.add_argument("URL", help="URL to delete product")
parser.add_argument("N", help="Number of products to delete")
# Defaults
...
def delete_n_products(process_id, url):
    """
    delete n products in the system in a strided manner
    :param url: URL to delete product
    :param start_id: start id of product
    :param n: number of products to delete after start_id

    This function is to be worked on by a process to attempt to delete all N
    ...
    for _i in range(start_id, n):
        ...
        if response.status_code != 200:
            print("Non 200 response deleting product:", _i, \
...
def main():
    """
    Main function to delete N products in the system.
    """
    ...
    print("Deleting", N, "products with 8 processes", file=sys.stderr)
    # Print out statistics
    print("Each process will delete ", N // NUM_THREADS, "products", file=sys.stderr)
    for i in range(NUM_THREADS):
        print("Process", i, "will delete products",
              i * (N // NUM_THREADS), "to", (i + 1) * (N // NUM_THREADS), file=sys.stderr)

    # Delete processes (one for each core on the machine)
    processes = []
    start_time = time.perf_counter()
    for i in range(NUM_THREADS):
        process = Process(target=delete_n_products, args=(i, URL+ENDPOINT))
        process.start()
    ...
    print(f"Deleted {N} products ({N} requests total)", file=sys.stderr)
