...
parser = argparse.ArgumentParser(description="Send N delete commands to URL/user at")
parser.add_argument("URL", help="URL to delete user")
parser.add_argument("N", help="Number of users to delete")
# Defaults
# URL to delete user
...
def delete_n_users(process_id, url):
    """
    delete n users in the system in a strided manner
    :param url: URL to delete user
    :param start_id: start id of user
    :param n: number of users to delete after start_id
...
def main():
    """
    Main function to delete N users in the system.
    """
    ...
    print("Each process will delete ", N // NUM_THREADS, "users", file=sys.stderr)
    for i in range(NUM_THREADS):
        print("Process", i, "will delete users",
    ...
    start_time = time.perf_counter()
    for i in range(NUM_THREADS):
        process = Process(target=delete_n_users, args=(i, URL+ENDPOINT))
        process.start()
    ...
    print(f"Deleted {N} users ({N} requests total)", file=sys.stderr)
